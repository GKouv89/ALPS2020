# Ανάπτυξη Λογισμικού για Πληροφοριακά Συστήματα 2020-21

Μέλη:

Γεωργία Κουτίβα, ΑΜ: 1115201700060
Αλέξανδρος Νεοφώτιστος, ΑΜ: 1115201700270

## Τελική Αναφορά
* Προδιαγραφές συστήματος στο οποίο έγιναν οι δοκιμές:
	* Επεξεργαστής: Intel Core i5-7200U CPU @ 2.50GHz
	* Εγκατεστημένη RAM: 6,00GB
	* Λειτουργικό Σύστημα: Ubuntu Bash Shell (ver. 20.04 LTS) on Windows 10 64bit
* Χρονικά Στατιστικά/Σύγκριση χρονικών απαιτήσεων παραδοτέου 2 με 3:
Αναλυτικότερα (η χρονομέτρηση έγινε με χρήση μεταβλητών time_t και της συνάρτησης time()):
* 2η εργασία: 
	* Total elapsed time:
		* Medium dataset: real 4m12.944s | user 1m2.297s | sys 1m35.094s
		* Large dataset: real 5m26.213s | user 1m46.953s | sys 1m34.125s
	* Αναλυτικότερα:
		* Medium dataset (χρόνος σε δευτερόλεπτα):
			* 120 for parsing and BOW creation
			* 1 for clique creation
			* 0 for IDF computation
			* 44 for tf-idf overall and average computation
    			* 2 for important word selection
			* 18 for sizing down TF-IDF array
    			* Training (5 epochs): 6 | Validation : 0 | Test: 0
		* Large dataset (χρόνος σε δευτερόλεπτα):
			* 82 for parsing and BOW creation
			* 3 for clique creation
			* 0 for IDF computation
			* 67 for tf-idf overall and average computation
    			* 2 for important word selection
			* 26 for sizing down TF-IDF array
    			* Training (5 epochs): 52 | Validation : 1 | Test: 2
 * 3η εργασία: 
	 * Total elapsed time:
		* Medium dataset: real 3m11.349s | user 1m2.672s | sys 1m30.328s
		* Large dataset:  real 3m17.780s | user 1m35.266s | sys 1m33.969s
	* Αναλυτικότερα:
		* Medium dataset (χρόνος σε δευτερόλεπτα):
			* 138 for parsing and BOW creation
			* 1 for clique creation
			* 0 for IDF computation
			* 68 for tf-idf overall and average computation
    			* 2 for important word selection
			* 20 for sizing down TF-IDF array
    			* 1 for batch files creation
    			* 5 for 5 epochs of training and one run on test file
		* Large dataset (χρόνος σε δευτερόλεπτα):
			* 78 for parsing and BOW creation
			* 2 for clique creation
			* 0 for IDF computation
			* 36 for tf-idf overall and average computation
    			* 2 for important word selection
			* 15 for sizing down TF-IDF array
    			* 5 for batch files creation
    			* 15 for 5 epochs of training and one run on test file
			* 4 for conflict resolution

* Στατιστικά Μνήμης: Καθώς το μεγαλύτερο από άποψη μνήμης τμήμα του προγράμματος είναι οι πίνακες BoW και TF-IDF, τα διαγράμματα μνήμης είναι σχεδόν ταυτόσημα και στα δύο παραδοτέα. Ενδεικτικά, το διάγραμμα μνήμης που εκτυπώνει ο massif για το 3ο παραδοτέο, medium dataset, είναι το εξής: 

```
    GB
11.76^                                                            ###
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#
     |                          ::::::::@::::::::::@:::::@::::::@:#  :
     |                       @::::::::::@::::::::::@:::::@::::::@:#  :
     |                      :@::::::::::@::::::::::@:::::@::::::@:#  :
     |                     @:@::::::::::@::::::::::@:::::@::::::@:#  :
     |                     @:@::::::::::@::::::::::@:::::@::::::@:#  :
     |                    :@:@::::::::::@::::::::::@:::::@::::::@:#  :
     |         ::@:::::::::@:@::::::::::@::::::::::@:::::@::::::@:#  :
     |        :::@:::::::::@:@::::::::::@::::::::::@:::::@::::::@:#  :
     |   @:::::::@:::::::::@:@::::::::::@::::::::::@:::::@::::::@:#  :
     | @@@:::::::@:::::::::@:@::::::::::@::::::::::@:::::@::::::@:#  :
   0 +----------------------------------------------------------------------->Gi
     0                                                                   174.4

```
			
* **Συμπεράσματα**:
	* Το βαρύτερο κομμάτι της εργασίας είναι μακράν η δημιουργία του BoW. Τα υπόλοιπα operations δεν είναι συγκρίσιμα στην διάρκειά τους, με εξαίρεση την δημιουργία του tf-idf array που συμπεριλαμβάνει περίπου 29800x26000 από το κάθε ένα από τα ακόλουθα: διαιρέσεις, προσθέσεις, αναθέσεις, πολλαπλασιασμούς + 26000 επιπλέον διαιρέσεις για τον υπολογισμό των σημαντικότερων λέξεων. 
	* Το sizing down του TF-IDF array, ενώ χρονοβόρο, απελευθερώνει χρειαζούμενη μνήμη, καθώς ο πίνακας είναι διάστασης περίπου 29800x26000, που πέφτουν σε μόλις 29800x1000 περίπου. 
	* Ακόμα και χωρίς πολυνηματισμό, ο χρόνος εκτέλεσης της διαδικασίας του machine learning φαίνεται ικανοποιητικός αν αναλογιστεί κανείς ότι για το μεγάλο dataset, μόνο το training set είναι 205.000 γραμμές περίπου, και τα epochs είναι 5. Συμπεραίνουμε ότι η δομή του hashmap είναι ιδιαίτερα χρήσιμη, καθώς σε γραμμικό με τα overflow lists χρόνο (των οποίων τα μεγέθη δεν έχουν πολύ μεγάλες αποκλίσεις) βρίσκουμε εύκολα ποια είναι τα διανύσματα που θα συμμετάσχουν στον υπολογισμό της σ(f(x)).
	* Με πολυνηματισμό, φαίνεται να πετυχαίνουμε μία καλή απόδοση, λαμβάνοντας υπόψιν μας και τον χρόνο διάσπασης των αρχείων σε batch files.
		* Με 4 νήματα (το μέγεθος της δέσμης είναι σταθερό και ίσο με 512), έχουμε 2,6 φορές γρηγορότερη εκτέλεση του κομματιού machine learning στο μεγάλο dataset.
		* Η εκτέλεση στα 8 νήματα αποφέρει παρόμοια επιτάχυνση.
		* Με 2 νήματα, έχουμε 18 δευτερόλεπτα χρόνο εκτέλεσης για τα 5 epochs του training και το ένα του testing, και 2 δευτερόλεπτα για την δημιουργία των αρχείων, άρα επιτάχυνση της τάξης του 2,7
		
    
## Νέες Προσθήκες - 3η Άσκηση
### Δομές
* **Job Scheduler**: Ο Job Scheduler κρατάει την ουρά των εργασιών που θα τρέξουν τα νήματα, καθώς και του σημαφόρους και τα condition variables που χρειάζονται για τον
	συγχρονισμό τους. Κρατά επίσης δείκτες στις κοινές δομές των hashmap και TF-IDF array, και ορισμένες μικρές δομές που αφορούν την εξαγωγή συνολικών αποτελεσμάτων
	από τους υπολογισμούς των νημάτων. Συγκεκριμένα:
	* Το πεδίο read_from_coeff_array, που υποδεικνύει αν βρισκόμαστε στην φάση του testing ή στην φάση του training. 
	* Ο πίνακας time_to_work δείχνει για κάθε νήμα αν περιμένει νέα αποτελέσματα από το master thread ή αν πρόκειται να πάρει κάποια εργασία. Αφού κάθε batch piece πρέπει
	να συνενωθεί στο master thread (είτε αυτό είναι το άθροισμα των coefficients και ο υπολογισμός νέων, είτε το άθροισμα των σωστών και των συνολικών προβλέψεων στην φάση 	του testing, τότε το thread αφού τελειώσει την εργασία που ανέλαβε πρέπει να θέσει το time to work του μηδέν, και όταν το master thread λάβει τα αποτελέσματα, αυτό θα ξανατεθεί ίσο με ένα ώστε το νήμα να γνωρίζει ότι ήρθε η ώρα για την επόμενη δουλειά. 
	* Ο πίνακας coefficients είναι τα βάρη που χρησιμοποιούνται στις προβλέψεις και που ανανεώνονται στο τέλος κάθε batch
	* O πίνακας threads_coeffs κρατά το άθροισμα των μερικών παραγώγων για το κάθε κομμάτι ενός batch. Έχει μία γραμμή για κάθε νήμα: Αυτές αθροίζονται και διαιρούνται τα στοιχεία τους με το πλήθος των γραμμών που διάβασαν τα νήματα για να υπολογιστούν τα νέα coefficients.
	* Ο πίνακας thread_predictions επιστρέφει το πλήθος των γραμμών που διάβασε το νήμα σε μία εκτέλεση της train και της test. Αυτά αθροίζονται στα συνολικά predictions στο testing για να υπολογιστεί το accuracy και στο training για να υπολογιστεί ο παρονομαστής στον υπολογισμό του μέσου λάθους.
	* Τα πεδία all_correct_predictions και all_predictions είναι αυτά που θα διαιρεθούν στο τέλος για να υπολογιστεί το accuracy στο training.
* **Queue** : Μία απλή ουρά που κρατά όλα τα jobs που θα εκτελέσουν τα προγράμματά μας.
* **Job** : Ένα struct στο οποίο κρατάμε το είδος ρουτίνας που αποτελεί η δουλειά (training, testing, validation) και το αρχείο με τα δεδομένα που αντιστοιχούν στην δουλειά.
* **Matchlist**: Μία λίστα που όσο κάνουμε το conflict resolution, κρατά για κάθε γραμμή στην οποία εμφανίζεται ένα δεδομένο file name το άλλο file_name στην γραμμή καθώς και
το prediction που έκανε το μοντέλο.

### Πως λειτουργούν οι νέες προσθήκες στο πρόγραμμά μας

1. Το μέγεθος δέσμης είναι 512 γραμμές. Τα threads είναι #define'd και είναι 4.
2. Για αρχή, οι νέες προσθήκες αντικαθιστούν τις κλήσεις από το αρχείο logistic regression. Πρώτα ανοίγει το αντίστοιχο training set (medium ή large) και σπάει σε κομμάτια που θα μοιραστούν στις δουλειές, ομοίως και με το testing set. Το σπάσιμο γίνεται με τρόπο που περιγράφεται στην Decrement Function.
3. Κατόπιν, τα αρχεία του training set μπαίνουν σε jobs 5 φορές στην ουρά για να αντιπροσωπεύσουν τα 5 epochs που τρέχει το πρόγραμμά μας. Τελευταία μπαίνουν σε jobs τα αρχεία
του testing, μία φορά το κάθε ένα στην ουρά. Καλείται η execute_all_jobs. Αυτή δημιουργεί τα νήματα και τα συγχρονίζει. Η λειτουργία της αναλυτικότερα έχει ως εξής:
	* Κάθε νήμα τρέχει την συνάρτηση threadWork. Η συνάρτηση αυτή αλληλεπιδρά με την execute_all_jobs/το main thread μέσω 2 condition_variables. Η πρώτη είναι η συνθήκη threads_complete != execution_threads. Ουσιαστικά, όσο δεν έχουν τελειώσει όλα τα νήματα να τρέχουν το δικό τους κομμάτι ενός batch, η condition variable αυτή περιμένει. Αντιθέτως, τα νήματα βγάζουν το κάθε ένα μία δουλειά από την ουρά (λειτουργία προστατευμένη από τον σημαφόρο queue_mutex), την εκτελούν, γράφουν τα αποτελέσματα στις αντίστοιχες δομές του scheduler και προσαυξάνουν την μεταβλητή threads_complete και θέτουν το time_to_work πεδίο τους ίσο με το μηδέν. Κάνουν signal την condition variable του master thread.
	* Η δεύτερη condition variable είναι η time_to_work == 0. Αν το νήμα έχει θέσει το πεδίο time_to_work του ίσο με το 0 περιμένει. 
	* Όταν όλα τα νήματα τρέξουν ένα κομμάτι του batch, τότε το threads_complete θα έχει γίνει ίσο με τον αριθμό των νημάτων. Σε αυτή την περίπτωση το master thread κάνει τους απαραίτητους υπολογισμούς: Αν είμαστε στην φάση του training υπολογίζει τα νέα coefficients, αν όχι αθροίζει τις σωστές και τις συνολικές προβλέψεις κάθε νήματος στον καθολικό μετρητή αυτών των δύο και θέτει το threads_complete = 0 και το time_to_work όλων των νημάτων = 1. Κάνει signal την condition variable της threadWork
	* Στο τέλος των while loops των δύο συναρτήσεων (ο κώδικας και στις δύο είναι μέσα σε while(1){} loop) ελέγχεται (με σημαφόρο) αν η ουρά των δουλειών είναι άδεια. Αν ναι, τότε τόσο τα επιμέρους threads, όσο και το master thread τερματίζουν.
4. Εκτυπώνεται το accuracy από το testing set, και καταστρέφεται ο job scheduler.	
	
	
* **Decrement function**: Η συνάρτηση Decrement είναι υπεύθυνη για τον διαχωρισμό ενός σετ σε μικρότερα batch files τα οποία θα δωθούν έπειτα στα threads για εκτέλεση εργασιών, κάθε ένα από αυτά θα έχει δηλαδή batchsize/execution_threads γραμμές, όπου **execution_threads** είναι το πλήθος νημάτων στα οποία θα σπάσει το batch file.
	* Αφού γίνει το χώρισμα πραγματοποιείται έλεγχος για τυχόν περισσευούμενα αρχεία, δηλαδή το πλήθος των αρχείων δεν διαρείται πλήρως με το δοθέν πλήθος execution threads. Στην περίπτωση που περισσεύουν αρχεία τοτε η Decrement:
		1) Ξεκινάει να μεταφέρει στοιχεία από τα (checking σε πληθος) έξτρα αρχεία σε ένα προσωρινό αρχείο, και διαγράφει αυτά τα οπόια το περιεχόμενο τους έχει αντιγραφεί.
		2) Υπολογίζεται εκ νέου ο όρος batchsize, όπου νέο batchsize = (πλήθος γραμμών που αντιγράψαμε από τα πλεονάζοντα αρχεία) / execution_threads
		3) Και τέλος μεταφέρει linesperbatch πλήθος γραμμών από το προαναφερθέν προσωρινό αρχείο στα νέα batch files.

### Conflict Resolution
Καθώς αντιμετωπίσαμε ορισμένα προβλήματα με το threshold με το οποίο πρέπει να γίνεται ο έλεγχος τόσο στο conflict resolution, όσο και στο testing, η πλήρης διαδικασία του
της σύγκλισης του conflict resolution δεν είναι ορατή μάλλον στο πρόγραμμά μας. Συγκεκριμένα, καθώς ακόμα και με batch training παρουσιάστηκε το πρόβλημα με τις προβλέψεις
του μοντέλου να είναι αποκλειστικά κοντά στο 0, δοκιμάσαμε να κάνουμε threshold tuning. Η διαδικασία που θα περιγράψουμε δοκιμάστηκε και στο validation αλλά και στο test set
για το medium dataset. Φτιάξαμε έναν πίνακα 1000 θέσεων και βάλαμε όλα τα thresholds με βήμα 0.001 από το 0 έως το 1 και δοκιμάσαμε ποιο από αυτά δίνει το καλύτερο *F-Score*.
Καθώς στην μία περίπτωση πήραμε αποτέλεσμα 0.077 και στην άλλη 0.078, το πρόγραμμα προσπαθεί να κάνει conflict resolution με βάση το threshold ίσο με 0.0775, καθώς με οποιοδήποτε από τα άλλα δύο δεν έβρισκε conflicts. Φυσικά, αυτή η παράμετρος ισχύει για το medium dataset. Το large dataset εξακολουθεί να μην βρίσκει κανένα conflict λόγω
του μεγάλου πλήθους προβλέψεων συγκεντρωμένων γύρω από 'ένα σημείο' (και στο medium dataset, λίγες ήταν οι προβλέψεις που ήταν μακριά από το 0.077 και το 0.078). Η διαδικασία που ακολουθείται για το conflict resolution είναι η εξής, και ελπίζουμε να γίνει πιο ξεκάθαρη μέσω 2 τεστ που γράψαμε για να δείξουμε την σκέψη μας.

Πρώτον, επαναρχικοποιείται η δομή των κλικών, δηλαδή διαγράφονται οι πληροφορίες για τα neglists, parents, next_in_clique από τους κόμβους του hashmap.

Γίνεται ένα πρώτο πέρασμα του validation set. Εκεί δημιουργούνται μόνο οι θετικές κλίκες: όσες προβλέψεις είναι πάνω από το threshold θα ακολουθήσουν την ίδια διαδικασία δημιουργίας θετικής κλίκας που ακολουθήθηκε και στο dataset W. Για κάθε μία από αυτές τις γραμμές, κρατάμε για κάθε ένα από τα δύο file names στην λίστα του ότι 'ταίριαξε' με το άλλο, και με ποια πιθανότητα το μοντέλο προέβλεψε ότι γίνεται αυτό. 

Γίνεται ένα δεύτερο πέρασμα και εκεί ακολουθείται η ίδια διαδικασία δημιουργίας αρνητικών κλικών με το dataset W, με μία διαφορά: όποια γραμμή θα μπορούσε να δημιουργήσει conflict (δηλαδή πάει μία κλίκα να πει ότι συσχετίζεται αρνητικά με τον εαυτό της) *αγνοείται* μέχρι νεοτέρας. Κρατάμε και πάλι τις αντιστοιχίες στα match lists των κόμβων κάθε γραμμής

Γίνεται το 3ο και τελευταίο πέρασμα του αρχείου. Αναγνωρίζουμε δύο περιπτώσεις δημιουργίας conflicts, και δύο τρόπους να λυθεί η κάθε μία.
**Πρώτη περίπτωση**: Μια πρόβλεψη λέει ότι δύο στοιχεία δεν ταιριάζουν, όμως αυτά βρίσκονται στην ίδια κλίκα. Σε αυτήν την περίπτωση, αυτά τα δύο στοιχεία θα έχουν έναν κοινό κόμβο στο match list τους, έτσι ώστε το κάθε ένα από αυτά να έχει προβλεφθεί από το μοντέλο ότι ταιριάζει με τον κοινό κόμβο, αλλά αυτά μεταξύ τους όχι. Εδώ, κοιτάμε την δεσμευμένη πιθανότητα των θετικών προβλέψεων που έκανε το μοντέλο: αν το γινόμενο των δύο πιθανοτήτων είναι μικρότερο από την διαφορά της μεγαλύτερης γενικά πρόβλεψης που έχει κάνει το μοντέλο με την πιθανότητα τα δύο στοιχεία να μην ταιριάζουν, τότε είναι πιο πιθανό αυτά τα στοιχεία πράγματι να μην ταιριάζουν, άρα μία από τις 2 ήδη προβλεφθείσες σχέσεις (όχι αυτή που δημιουργεί το conflict) είναι λανθασμένη. Η μικρότερη από τις δύο παλιές προβλέψεις 'διορθώνεται', δηλαδή αθροίζουμε σε έναν πίνακα το γινόμενο του διανύσματος των δύο αρχείων με την cross entropy. Αν το γινόμενο είναι μεγαλύτερο, τότε η γραμμή που δημιουργεί το conflict είναι η λάθος και 'διορθώνεται' ώστε τα δύο στοιχεία να ταιριάζουν
**Δεύτερη περίπτωση**: Μία πρόβλεψη λέει ότι δύο στοιχεία ταιριάζουν, όμως αυτά βρίσκονται σε διαφορετικές κλίκες. Σε αυτήν την περίπτωση, ελέγχουμε τι είναι πιο πιθανό: να μην ταιριάζουν οι αντιπρόσωποι των κλικών ή να ταιριάζουν αυτά τα δύο στοιχεία; Όποιο 'νικήσει' αναγκάζει το άλλο σε διόρθωση. **ΠΡΟΣΟΧΗ: το πρόγραμμα δεν φαίνεται να βρίσκει καμία τέτοια περίπτωση conflict, άρα ίσως δεν είναι έγκυρη**

Διαιρούνται όλα τα στοιχεία του πίνακα στον οποίον αθροίζαμε τις διορθώσεις κάθε φορά που συναντούσαμε conflict με των συνολικό αριθμό conflicts που συναντήσαμε, και πολλαπλασιάζονται με το learning rate. Τέλος, γίνεται η διόρθωση των coefficients, καταστρέφονται οι πληροφορίες για τις κλίκες, και γίνεται νέος έλεγχος.
Στο κανονικό πρόγραμμα, αυτό οδηγεί σε μη εύρεση νέων conflicts στην δεύτερη επανάληψη, καθώς οι περισσότερες προβλέψεις τώρα είναι μεγαλύτερες από το threshold και άρα δεν υπάρχει περιοχή επικάλυψης αυτού με τις περισσότερες προβλέψεις. Εμείς γράψαμε τα δύο ακόλουθα τεστ ώστε να δείξουμε πως θεωρήσαμε ότι η σκέψη μας μπορεί να είναι εφαρμόσιμη. 

Τα τεστ αυτά απαιτούν ορισμένες διαγνωστικές εκτυπώσεις τις οποίες δεν θα μπορούσαμε να αφήσουμε μέσα στο πρόγραμμα, καθώς το output αυτών θα κάλυπτε το χρήσιμο output που προέκυψε από το πρώτο παραδοτέο. Τα δύο tests έχουν τους ίδιους τρεις κόμβους που συναντώνται στα περιεχόμενα του αρχείου conflicts.txt στον φάκελο tests/multithreading_tests και απλά διαφέρει το tf-idf τους. Στην πρώτη περίπτωση προσομοιώνουμε την πρώτη περίπτωση σύγκρουσης, και την πρώτη λύση: Είναι πιο πιθανό η κλίκα να πρέπει να σπάσει παρά τα στοιχεία της γραμμής που δημιουργεί την σύγκρουση να ταιριάζουν. Προσθέτοντας εκτυπώσεις για τις προβλέψεις, παίρνουμε το εξής αποτέλεσμα:
```
case1 solution1
prediction1 = 0.6049181019632078, prediction2 = 0.5314583915563169, prediction = 0.2141650169574414
Total conflicts found in this run       1
case1 solution1
prediction1 = 0.5876193157576278, prediction2 = 0.5160637650518647, prediction = 0.2022153494407017
Total conflicts found in this run       1
case1 solution1
prediction1 = 0.5706050087211323, prediction2 = 0.5010798594667485, prediction = 0.1910908665890886
Total conflicts found in this run       1
Total conflicts found in this run       0
```

Το πρόγραμμα συγκλίνει μετά από ένα σύντομο πλήθος επαναλήψεων, και μειώνει την πιθανότητα των αρχείων να ταιριάζουν.
Στην δεύτερη περίπτωση προσομοιώνουμε την δεύτερη περίπτωση σύγκρουσης και την δεύτερη λύση: Είναι πιο πιθανό τα στοιχεία να ταιριάζουν παρά να πρέπει να σπάσει η κλίκα. Προσθέτοντας εκτυπώσεις για τις προβλέψεις, παίρνουμε το εξής αποτέλεσμα:

```
case1 solution2
prediction1 = 0.6049181019632078, prediction2 = 0.6739268871011302, prediction = 0.3318122278318339
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.6238311804666662, prediction2 = 0.6902856559456821, prediction = 0.3514584790083549
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.6418346542571942, prediction2 = 0.7057209952467893, prediction = 0.3710213640759055
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.6589216997943398, prediction2 = 0.7202534336011545, prediction = 0.3904077254843227
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.6750976828782648, prediction2 = 0.7339110677486943, prediction = 0.4095334267386244
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.6903781276366613, prediction2 = 0.7467277593662325, prediction = 0.4283243635874538
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.7047867283030764, prediction2 = 0.7587415032222627, prediction = 0.4467169841149055
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.7183534863030707, prediction2 = 0.7699929979514575, prediction = 0.4646583715704384
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.7311130273533252, prediction2 = 0.7805244317952370, prediction = 0.4821059623007026
Total conflicts found in this run       1
case1 solution2
prediction1 = 0.7431031289212905, prediction2 = 0.7903784810396786, prediction = 0.4990269795305122
Total conflicts found in this run       1
Total conflicts found in this run       0
```
Αυξάνεται η πιθανότητα των στοιχείων να ταιριάζουν (3η στήλη).

### Unit Testing
* Δοκιμάζεται ολόκληρη η διαδικασία με μικρά αρχεία. Έχουμε 4 training files, με δύο γραμμές το καθένα - ουσιαστικά 2 batches των 4 γραμμών - και 2 testing files - 1 batch των 4 γραμμών. Οι δουλειές δημιουργούνται, μπαίνουν στην ουρά και μετά υπολογίζονται σειριακά τόσο τα coefficients όσο και το accuracy. Και τα δύο ταυτίζονται με τα αποτελέσματα της παράλληλης εκτέλεσης.  
* Επίσης, δοκιμάζεται η δημιουργία των batch files και η είσοδος στην ουρά των training files σε 5 epochs και των testing files σε ένα.  

## Νέες Προσθήκες - 2η Άσκηση

### Δομές
 * **Stopword List**: Μία απλή συνδεδεμένη λίστα που περιέχει τις λέξεις που δίνονται στο αρχείο που παρέχεται στην εκφώνηση της εργασίας. 
 * **Vector**: Ένα απλό διάνυσμα με στοιχεία ακέραιους αριθμούς. Ο τύπος αυτός χρησιμοποιείται για όλα τα διανύσματα του Bag of Words και για το πρώτο διάνυσμα στο TF-IDF array. 
 * **IDFVector**: Ένα απλό διάνυσμα με στοιχεία αριθμούς κινητής υποδιαστολής διπλής ακρίβειας. Χρησιμοποιείται για τα διανύσματα του TF-IDF array εκτός από το πρώτο.
 * **Dictionary**: Ένα απλό δυαδικό δένδρο αναζήτησης. Οι λέξεις εισάγονται με κριτήριο την αλφαβητική τους σειρά. Οι κόμβοι του δένδρου, πέρα από δείκτες στο αριστερό και δεξί υποδένδρο τους, περιέχουν και το payload της λέξης: ένας αύξων αριθμός που υποδεικνύει την σειρά εισαγωγής της λέξης στο δένδρο. 
 * **Bag Of Words**: Ένας πίνακας από σταθερό πλήθος Vectors (ίσο με τον αριθμό των αρχείων στο dataset X, συν άλλο ένα που περιέχει τα payloads των λέξεων, όπως αυτές εμφανίζονται στο λεξικό).
 * **TF**: Ένας πίνακας που περιέχει τις τιμές TF-IDF για όλα τα κείμενα που περιέχονται στο dataset X. Περιέχει ένα απλό Vector, με τα payloads των λέξεων όπως αυτές εμφανίζονται στο λεξικό, και σταθερό πλήθος IDFVectors που περιέχουν τα TF-IDF values για κάθε κείμενο.
* **neg_list**: Μια λίστα που περιέχει τους κόμβους αρνητικής συσχέτισης (neg_nodes) ενός αντιπροσώπου θετικής κλίκας.
	* Ο όρος Αρνητική Συσχέτιση θα χρησιμοποιείται για στοιχεία μεταξύ των οποίων το matching value είναι 0.
* **neg_node**: Αντιπροσωπεύει έναν κόμβο αντιπρόσωπο θετικής κλίκας, ο οποίος έχει αρνητική συσχέτιση με έναν άλλο αντιπρόσωπο θετικής κλίκας. Μέσα περιέχεται ο “αρνητικός” αντιπρόσωπος μιας άλλης θετικής κλίκας, δείκτης για τον επόμενο της λίστας καθώς και το flag printed το οποίο χρησιμοποιείται για αποφυγή εκτύπωσης διπλοτύπων (η χρήση της αναφέρεται στην συνέχεια).
* Περισσότερα για το πως τεστάρουμε τις νέες δομές στην ενότητα Unit Testing.

### Πως λειτουργούν οι νέες προσθήκες στο πρόγραμμά μας

 1. Για αρχή, δημιουργείται το stopword list. 
 2. Στον parser, ο οποίος διαβάζει τα αρχεία και εξακολουθεί να κρατά την tuple list, έχει ενσωματωθεί η δημιουργία του Bag of Words και του Λεξικού.
	 * Κάθε ένα από τα buffer που περιέχουν είτε το όνομα ενός attribute είτε την τιμή του δίνεται στην συνάρτηση bow_it. Αυτή με την σειρά της χωρίζει το string στο whitespace και αφαιρεί αριθμούς και σημεία στίξης, καθώς μετατρέπει επίσης όλα τα γράμματα σε lowercase. 
	 * Όταν συναντήσει whitespace και έχει επομένως διαβάσει μία λέξη, πάει να την εισάγει στο δένδρο. Αν αυτή ήδη υπάρχει, βρίσκει το payload της.
	 * Αν η λέξη δεν υπήρχε στο λεξικό, τότε πρέπει να προστεθεί μία νέα λέξη στο Bag of Words. Το payload προστίθεται στο πρώτο διάνυσμα και, αν αυτό έφτασε στην μέγιστη χωρητικότητά του, διπλασιάζεται σε μέγεθος τόσο αυτό όσο και τα υπόλοιπα διανύσματα του BoW. Κατόπιν, η κατάλληλη θέση του διανύσματος /κειμένου το οποίο επεξεργαζόμαστε γίνεται 1 (1η εμφάνιση της λέξης)
	 * Αν η λέξη υπήρχε στο λεξικό, τότε βρίσκουμε την θέση payload στο τρέχον διάνυσμα και αυξάνουμε την συχνότητα εμφάνισης κατά 1.
	 * Αν η λέξη δεν είχε ξαναεμφανιστεί στο κείμενο, αυξάνουμε το word_count του διανύσματος. Αυτό θα μας χρησιμεύσει στον υπολογισμό του TF.
	 * Παράλληλα, δημιουργούμε και το διάνυσμα που περιέχει τα idf values για όλες τις λέξεις.
		 * Για κάθε κείμενο στο οποίο εμφανίζεται μία λέξη, αυξάνουμε το αντίστοιχο στοιχείο ενός  IDFVector κατά 1.
3. Μετά την λήξη του parsing, αντιστρέφουμε τα στοιχεία του διανύσματος του IDFVector, πολλαπλασιάζουμε με το πλήθος των αρχείων, και λογαριθμούμε - υπολογίζουμε τα IDF values των λέξεων.
4. Κατόπιν, φτιάχνουμε τον πίνακα TF-IDF. Διαιρούμε τα στοιχεία κάθε vector του BoW με το word_count του vector και πολλαπλασιάζουμε με το IDFVector. 
5. Ταυτόχρονα, υπολογίζουμε το μέσο TF-IDF για κάθε λέξη
	* Αφού υπολογιστεί αυτό, αποδεσμεύουμε το BoW.
6. Αντιγράφουμε το Vector του TF-IDF πίνακα με τα payloads των λέξεων και ταξινομούμε τόσο αυτό όσο και το IDFVector με τα μέσα TF-IDF σε φθίνουσα σειρά.
	* Χρησιμοποιείται ο αλγόριθμος Quicksort.
7. Κάνουμε crop το Vector με τα payloads, κρατώντας τα πρώτα IMPWORDS στοιχεία (ορισμένο στα 1000)
8. Κάνουμε sort το Vector αυτό με αύξουσα σειρά αυτή την φορά. Γνωρίζουμε επακριβώς τις στήλες που θα κρατήσουμε μετά το clipping του TF-IDF πίνακα
9. Δημιουργούμε έναν νέο πίνακα τύπου TF με ίδιο πλήθος διανυσμάτων ίσο με του παλιού και νέο μήκος διανυσμάτων ίσο με IMPWORDS. Αντιγράφουμε τις στήλες του παλιού πίνακα που αναγράφονται στο Vector που ταξινομήσαμε και αποδεσμεύουμε το  παλιό TF.
10. Ξεκινά η διαδικασία του training. Κάνουμε 5 epochs επάνω στο training set: βρίσκουμε μέσω του hashmap τον αριθμό vector του κάθε JSON file μέσα στον TF-IDF πίνακα, κάνουμε concatenate τα 2 διανύσματα, και υπολογίζουμε την τιμή σ(f(x)). Ανανεώνουμε τις συνιστώσες (ένας απλός πίνακας στοιχείων double) με βάση τον τύπο *(prediction - ground truth) x learning rate x xi* όπου xi η i-οστή τιμή TF-IDF του concatenated διανύσματος. Μετά από κάθε epoch, γίνεται μία επανάληψη του Validation Set. Εκτυπώνονται ορισμένα στατιστικά τόσο κατά την διάρκεια του validation όσο και κατά το testing (θα αναφερθούμε σε αυτά σύντομα). Τέλος, γίνονται προβλέψεις επάνω στο  Test Set. 
	* Τα Training, Validation, και Test sets είναι μερίδια derived dataset που προκύπτει από τον 'υπολογισμό' θετικών και αρνητικών κλικών, μεγέθους 60%, 20% και 20%,  αντίστοιχα.

### Εκτύπωση κλικών αρνητικής συσχέτισης αντιπροσώπου και η μεταβλητή printed:
* Κατά την δημιουργία κάθε κόμβου list_node, η μεταβλητή printed αρχικοποιείται με  1.
* Κατά την διάρκεια της συνάρτησης neglist_print, πριν ξεκινήσει η εκτύπωση, πραγματοποιείται ελεγχος της τιμής της μεταβλητής printed, η οποία καθορίζει αν ο συγκεκριμένος αντιπρόσωπος και η θετική κλίκα του, θα εκτυπωθούν.
	* Αν η printed του δεύτερου αντιπροσώπου είναι 1 τότε, αναζητούμε μέσα στους κόμβους της λίστας αρνητικής συσχέτισης του, τον πρώτο αντιπρόσωπο, θέτουμε την printed του 0 και συνεχίζουμε με την εκτύπωση.
	* Διαφορετικά, αν η printed είναι 0 τότε δεν γίνεται εκτύπωση και η συνάρτηση συνεχίζει στον επόμενο αντιπρόσωπο στην λίστα αρνητικής συσχέτισης με παρόμοιο τρόπο.
* Πρώτα θα εκτυπωθούν οι  δυο αντιπρόσωποι κλικών που περιέχουν στοιχεία που γνωρίζουμε οτι είναι διαφορετικά μοντέλα μεταξύ τους, στην συνέχεια ο πρώτος αντιπρόσωπος θα εκτυπωθεί με τα στοιχεία της δεύτερης κλίκας και τέλος τα στοιχεία της κλίκας του πρώτου αντιπροσώπου θα εκτυπωθούν με τον δεύτερο αντιπρόσωπο και τα στοιχεία της κλίκας αυτου.

### Τυχαιοποίηση Αποτελεσμάτων
Αφού τελειώσει το χώρισμα των αποτελεσμάτων στα αρχεία θετικών και αρνητικής συσχέτισης κλικών, το splitter.c αναλαμβάνει να συμπτύξει τα δύο προαναφερθέντα αρχεία σε ένα αρχείο που θα περιέχει όλες τις εισόδους των προηγουμένων με τυχαία σειρά, ενώ παράλληλα με την χρήση μιας rand γίνεται η επιλογή του σετ (training/validation/test) στο οποίο θα πάει.

```
void print_all_cliques(int opt, clique_list* l){
    clique_list_node *temp = l->front;

    int posc = 0;

    FILE *fp1; // for positive relationship cliques
    FILE *fp2; // for negative relationship cliques
    
    // Preparing files for result splitting 
    fp1 = fopen("poscliques.csv","w+");
    fp2 = fopen("negcliques.csv","w+");
    
    if(fp1 == NULL){
        fprintf(stderr, "Couldn't open poscliques.csv file.\n");
    }
    if(fp2 == NULL){
        fprintf(stderr, "Couldn't open negcliques.csv file.\n");
    }
    
    printf("left_spec_id, right_spec_id\n");
        while(temp){
            print_clique(opt, temp->representative, fp1);
            temp = temp->next;
        }
        temp = l->front;
        while(temp){
            neglist_print(temp->representative, fp2);
            temp = temp->next;
        }
    
    if(fclose(fp1)!= 0){
        fprintf(stderr, "Couldn't close poscliques.csv file.\n");
    }
    if(fclose(fp2)!= 0){
        fprintf(stderr, "Couldn't close negcliques.csv file.\n");
    }
}
```
***ΣΗΜΕΙΩΣΗ***: Αυτή η διαδικασία έγινε μια φορά για να δημιουργηθούν τα 3 σετς, για αυτό και η μη διατήρηση των δυο file pointers στην print_all_cliques.

### Μερικά συμπεράσματα για το Training

 - Επιλέξαμε την λήξη του training κατόπιν συγκεκριμένου αριθμού από epochs έναντι της σύγκλισης της διαφοράς των νέων με των παλιών συντελεστών καθώς παρατηρήσαμε ότι με τον δεύτερο τρόπο η διαδικασία έπαυε πολύ πριν γίνει το πρώτο πέρασμα του training set, ακόμα και αν το ε που ορίζαμε ήταν 0.001.
 - Η επιλογή του συγκεκριμένου threshold (=0.00041) έγινε μετά από απάντηση σε σχετική ερώτηση συναδέλφων στο Piazza (συγκεκριμένα, αναφερόμαστε στην ερώτηση 67). Παρατηρήσαμε τα εξής:
	 - Αρχικά, όσο διαβάζουμε τόσο label = 1 όσο και label = 0, οι τιμές είναι πιο κοντά στο 0.4. Όμως, καθώς υπάρχουν πολλά περισσότερα μηδενικά στο τέλος του αρχείου (όπου η εκτύπωση άσσων έχει εξαντληθεί), οι αριθμοί πέφτουν πάρα πολύ και πλησιάζουν το 0.0005.
	 - Επίσης, ο λόγος γραμμών με label = 1 προς γραμμές με label = 0 ήταν πάρα πολύ μικρός (για την ακρίβεια κοντά στο 0.07 για το test set  και κοντά στο 0.08 για το validation set).
	 - Επομένως, πειραματιστήκαμε με τιμές κοντά στο 0.0005. Καταλήξαμε στο συγκεκεριμένο threshold διότι:
		 -  Η τιμή για την οποία ο αριθμός σωστών προβλέψεων για label = 1 και ο αριθμός σωστών προβλέψεων για label = 0 ήταν ο πιο ισορροπημένος, και πλησιέστερος στο συνολικό ποσοστό accuracy. (Πάλι, ο αριθμός σωστών προβλέψεων για label = 1 ήταν χαμηλότερος, αλλά ελαφρά)
		 - Από περίπου 57% accuracy για το συγκεκριμένο threshold, πέφτουμε στο 42% για threshold = 0.00040, ενώ για αύξηση στο 0.00042 ανεβαίνουμε μόλις 3%. Ανάλογα μικρές είναι οι αυξήσεις κατά 0.00001 έως ότου φτάσουμε το 0.0005 όπου η ακρίβεια είναι ίδια σχεδόν με το 0.5 (δηλαδή ελάχιστες προβλέψεις μεγαλύτερες του 0.0005). Θεωρήσαμε ότι αυτή η απότομη πτώση στην απόδοση υποδεικνύει το threshold που πρέπει να πάρουμε λόγω του bias που έχει εισαχθεί στο μοντέλο. 
	 - Αν επιλέξετε να τρέξετε το πρόγραμμά μας για threshold = 0.5, θα δείτε ότι το ποσοστό επιτυχίας είναι 92% - μαντεύει σωστά όλα τα μηδενικά και κανέναν άσσο. Αν τρέξετε το πρόγραμμά μας για threshold = 0.00041 έχουμε accuracy περίπου 57% στο validation set (across all epochs) με περίπου 52% σωστές προβλέψεις για label = 1 και 56% σωστές προβλέψεις για label = 0. Παρόμοια αποτελέσματα λαμβάνουμε και για το test set. 
 
 Οι παραπάνω πειραματισμοί αφορούν to medium dataset. Για το large dataset το bias υπερ των 0 είναι προφανές χωρίς να αλλάξει το threshold, ακόμα και αν ο λόγος 1 προς 0 είναι μεγαλύτερος απ'ότι στο medium. Ο λόγος εξακολουθεί να είναι οι πολλαπλές αρνητικές συσχετίσεις στο training set που καθαρά λόγω πλήθους κατεβάζουν τα αποτελέσματα της σ(f(x)) ακόμα πιο κοντά στο 0 απ'ότι γινόταν στο medium dataset. 

 

 

## 1η Άσκηση

### Οι δομές μας και τι αντιπροσωπεύουν

* list_node: Αντιπροσωπεύει ένα αρχείο .json, με το id του (με την μορφή με την οποία εμφανίζεται στα .csv αρχεία), τα attribute/key pairs του, και κάποιους δείκτες που θα εξηγηθούν παρακάτω
* info_list: Μία λίστα από list_nodes. Υπάρχει μία τέτοια για κάθε bucket του hashmap μας.
* node_tuple: Ένας κόμβος που περιέχει ένα attribute/key pair, μία γραμμή σε κάποιο αρχείο .json. Συνδέεται με τις υπόλοιπες γραμμές του εν λόγω αρχείου.
* tuplelist: Μία λίστα από node_tuples. Υπάρχει μία τέτοια για κάθε list_node. Καθώς δεν χρησιμοποιήσαμε το description των json files τους, αλλά μόνο το url/id τους, καταχωρίσαμε αυτά τα ζευγάρια σε μία απλή λίστα.
* hash_map/hash_bucket: Υλοποίηση ενός τυπικού στατικού πίνακα κατακερματισμού. Κάθε bucket περιέχει μία λίστα των list_nodes των οποίων τα ids αντιστοιχούν σε αυτόν.
	* Η επιλογή της συνάρτησης κατακερματισμού, καθώς και του αριθμού των buckets, έγινε μετά από αναζήτηση στο διαδίκτυο, εφόσον αναφέρθηκε ότι επιτρέπεται.
		* Διαβάσαμε πως είναι καλό κάθε χαρακτήρας του string id να συνεισφέρει στο αποτέλεσμα της hash function, 
		* καθώς και πως ο πολλαπλασιασμός με πρώτο και η επιλογή πρώτου αριθμού για αριθμό buckets δίνουν καλύτερα αποτελέσματα.
			* Σχετικές πηγές: https://stackoverflow.com/questions/2624192/good-hash-function-for-strings
			* https://computinglife.wordpress.com/2008/11/20/why-do-hash-functions-use-prime-numbers/
		* Μέσω της συνάρτησης print_bucket_no_of_entries, παρατηρήσαμε ότι ο μέσος όρος entries/bucket ήταν λογικός, καθώς και ότι δεν υπήρχαν τρομερές αποκλίσεις από αυτών.
		* Επιλέξαμε αυτή την δομή καθώς 'σπάει' το dataset σε πολλά μικρότερα κομμάτια που δεν είναι δύσκολο να διατρεχθούν γραμμικά.

Η δομή που επιλέχθηκε για την δημιουργία των κλικών είναι στην πραγματικότητα τα disjoint sets. Δεν υπάρχει ξεχωριστή δομή από το hashmap: κάθε spec σε μία γραμμή
στα .csv files με label = 1 περνά από την hash_function, βρίσκουμε τον αντίστοιχο κόμβο στην λίστα του bucket, και επάνω σε έναν δείκτη σε αυτόν τον κόμβο κάνουμε
πράξεις disjoint sets ανάλογες με την υλοποίηση των ανοδικών δένδρων. Το πεδίο parent σε έναν list_node είναι αυτό που χρησιμοποιείται για την υλοποίηση της εύρεσης ρίζας και της ένωσης συνόλων.
Όμως, για επιτάχυνση της εκτύπωσης των αποτελεσμάτων κρατάμε μία λίστα με τους αντιπροσώπους των συνόλων/κλικών. Αυτή είναι η clique_list. Τα nodes της είναι
οι αντιπρόσωποι των disjoint sets/cliques και το πεδίο next_in_line ακολουθείται για τον καθέναν από αυτούς, σχηματίζοντας μία λίστα με όλους τους κόμβους που
ανήκουν στην κλίκα. Αυτό το πεδίο ανανεώνεται με κάθε ένωση συνόλων.

### Πως λειτουργεί το πρόγραμμά μας

Πρώτα, γίνεται το parsing του dataset X. Δημιουργείται το list_node, το tuplelist για κάθε αρχείο και ο κόμβος προστίθεται στον hashmap.
Μετά, γίνεται το parsing του dataset W ή Υ, ανάλογα με το πιο έχει επιλεχθεί (περισσότερα για αυτό σύντομα). Σε κάθε γραμμή με label = 1, βρίσκουμε τους κόμβους
μέσα στο hashmap των left_spec, right_spec και κάνουμε ένωση των κλικών στις οποίες ανήκουν. Τέλος, εκτυπώνεται η λίστα με όλες τις κλίκες.

## Πως να τρέξετε το πρόγραμμά μας

Το path για τα dataset είναι defined, και πρέπει και τα 3 να βρίσκονται στον ίδιο φάκελο με τα .c αρχεία μας.

* Για να μεταγλωττίσετε το πρόγραμμα με σκοπό να τρέξει για το medium dataset,
        ```
                make medium
        ```

* Για να μεταγλωττίσετε το πρόγραμμα με σκοπό να τρέξει για το large dataset,
        ```
                make large
        ```
* Για να τρέξετε το πρόγραμμα με έξοδο στο stdout,
        ```
                make run
        ```
* Για να τρέξετε το πρόγραμμα με έξοδο στο αρχείο output.txt
        ```
                make run_out_file
        ```
## Unit testing

* Για τον parser του dataset X, δεν υπάρχει κάποιο διαθέσιμο unit test, ο έλεγχος για την ορθότητά του έγινε χειρωνακτικά μέσω αλλεπάλληλων εκτυπώσεων.

* Για το map και το info_list, το testing είναι trivial

* Για τις κλίκες, δοκιμάζεται:
        * Η δημιουργία μίας κλίκας από ορισμένα test_vectors
        * Η εισαγωγή και η διαγραφή από την λίστα των αντιπροσώπων κλικών (όταν δύο κλίκες συνενώνονται, η μεγαλύτερη 'απορροφά' την μικρότερη και έτσι ο αντιπρόσωπος αυτής αφαιρείται από την συνολική >        * Η δημιουργία δύο κλικών από ορισμένα test_vectors

* Για τις αρνητικές κλίκες, δοκιμάζεται:
	* Η δημιουργία θετικής και αρνητικής κλίκας από ορισμένα test_vectors
	* Η εισαγωγή κόμβων-αντιπροσώπων της θετικής κλίκας, στην κλίκα αρνητικών συσχετίσεων ενός αντιπροσώπου

* Για τις αρνητικές κλίκες, δοκιμάζεται:
	* Η δημιουργία θετικής και αρνητικής κλίκας από ορισμένα test_vectors
	* Η εισαγωγή κόμβων-αντιπροσώπων της θετικής κλίκας, στην κλίκα αρνητικών συσχετίσεων ενός αντιπροσώπου

* Για το parsing των csv αρχείων, γίνονται έλεγχοι ότι διαβάζονται όλες οι γραμμές (φτάνουμε στο EOF με ασφάλεια), καθώς και ότι ο αριθμός γραμμών με label = 1 είναι ο αναμενόμενος.
* Για το BoW, τον πίνακα TF-IDF, και τις συναρτήσεις του logistic regression, δημιουργήσαμε ένα παράδειγμα με 2 test vectors που περιείχαν payloads λέξεων. Δοκιμάζεται:
	* Η εισαγωγή νέων λέξεων/payloads στο bag of words, καθώς και λέξεων που εμφανίζονται πάνω από μία φορά.
	* Η καταγραφή των λέξεων που κάθε κείμενο περιέχει.
	* Ο αριθμός εμφανίσεων κάθε λέξης σε κάθε κείμενο.
	* Η τιμή των TF-IDF διανυσμάτων (δεν επιβεβαιώνεται μέσω assert, λόγω της επιλογής αριθμών διπλής ακρίβειας, αλλά έχει επιβεβαιωθεί χειρωνακτικά από εμάς)
	* Ο υπολογισμός του μέσου TF-IDF
	* Η ταξινόμηση των διανυσμάτων μέσου TF-IDF (με φθίνουσα σειρά) και payloads σημαντικότερων λέξεων (με αύξουσα σειρά)
	* Η ισότητα των στηλών που κρατούνται στο νέο, μικρό IDF με τις αρχικές στήλες.
	* Η σωστή τιμή των συναρτήσεων σ(f(x)) και η σωστή ανανέωση των συνιστωσών (αυτά τα αποτελέσματα δεν επιβεβαιώνονται μέσω assert, λόγω της επιλογής αριθμών διπλής ακρίβειας, αλλά έχουν επιβεβαιωθεί χειρωνακτικά από εμάς).
* Για το Λεξικό τεστάρουμε την εισαγωγή λέξεων στο δένδρο, την αποφυγή εισαγωγής μίας λέξης που ήδη υπάρχει, και αν το payload που επιστρέφεται από αναζήτηση στο δένδρο μίας λέξης που εμφανίζεται πάνω από μία φορές σε ένα σύνολο λέξεων είναι αυτό που δίνεται στην πρώτη απόπειρα εισαγωγής της λέξης.

Για να τρέξετε όλα τα tests, αρκεί να τρέξετε:
        ```
                make run_all_tests
        ```

Τέλος,
	```
        make clean
        ```
διαγράφει το εκτελέσιμο του προγράμματος και
        ```
                make clean_tests
        ```
διαγράφει τα εκτελέσιμα των tests.
